...
namespace Entidades
{
    ...
    public sealed class CentroMedico : ISerializable, IDeserializable<CentroMedico>
    {
        ...
        /// <summary>
        /// Devuelve el proximo id a asignarle al turno.
        /// </summary>
        /// <exception cref="Exception"></exception>
        public int ProximoIdTurno
        {
            get
            {
                int id = 0;
        ...
        public int ImportarPacientes(string fullPath)
        {
            try
            {
                ...
                {
                    if (fullPath.Contains(".xml"))
                        lista = this.DeserializarListaFromXML(fullPath, this.Pacientes);

                    else if (fullPath.Contains(".json"))
                        lista = this.DeserializarListaFromJSON(fullPath, this.Pacientes);
        ...
        public int ImportarProfesionales(string fullPath)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(fullPath))
                {
                    ...
                    if (fullPath.Contains(".xml"))
                        lista = this.DeserializarListaFromXML(fullPath, this.Profesionales);


                    else if (fullPath.Contains(".json"))
                        lista = this.DeserializarListaFromJSON(fullPath, this.Profesionales);
        ...
        public static bool operator -(CentroMedico centro, Persona persona)
        {
            try
            {
                ...
                {
                    if (centro == persona)
                        return centro.Profesionales.Remove((Profesional)persona); ;
                }
